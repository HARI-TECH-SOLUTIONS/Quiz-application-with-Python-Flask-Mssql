...
def login():
    ...
    if request.method == 'GET':
        ...
        logger.warning("-----> Result: {}, name : {}".format(result, sname))
        ...
        if result:
            ...
            if result.role == 'student':
                ...
                return redirect(url_for('student_home', sname=sname))

            elif result.role == 'teacher':

                flash('You were successfully logged in as Teacher')
                # return render_template('teacher_home.html', sname=sname)
                return redirect(url_for('teacher_home', sname=sname))
...
@app.route('/teacher_home/<sname>')
@login_required
def teacher_home(sname):
...
@app.route('/student_home/<sname>')
@login_required
def student_home(sname):
...
def result_studentquiz():
    ...
    if request.method == 'POST':
        ...
        score = round(
            (len([i for i in selected_ans if i == 'True']) / qauestion_count) * 100.0, 2)
