...
def checkUser(usn, psw):
    ...
    try:
        ...
        logger.info("student details : {}".format(result))
...
def studentQuizQuestions(qu_id, cid):
    try:
        ...
        result = db.query(questions).filter(
            and_(questions.c.course_id == cid, questions.c.quiz_id == qu_id)).all()
        ...
        if result:
            ...
            result1 = db.query(answers).filter(
                answers.c.q_id.in_(qes_id)).all()
...
def get_question_count(courseid, quizid):
    ...
    result = db.query(questions).filter(
        and_(questions.c.course_id == courseid, questions.c.quiz_id == quizid)).all()
